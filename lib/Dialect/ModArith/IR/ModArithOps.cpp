...
LogicalResult verifySameWidth(OpType op, ModQTypeInterface modularType,
  ...
  auto storageType = dyn_cast<IntegerType>(modularType.getStorageType());
  if (!storageType) {
    return op.emitOpError()
           << "expected modular type storage type to be an integer type";
  }
...
LogicalResult verifyTypeLowering(OpType op, Type modularType,
  ...
  int64_t rnsLength = (*modQType).getNumResidues();
  if (rnsLength == 1) {
    rnsLength = 0;
  }
